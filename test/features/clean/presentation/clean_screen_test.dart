...
void main() {
  testWidgets('says plainly that a preview removes nothing', (tester) async {
    await tester.pumpWidget(harness(_FakeCleanRepository([
      const CleanPlan(candidates: [
        CleanCandidate(path: '/a/cache', section: 'User essentials', sizeBytes: 2048),
      ]),
    ])));
  ...
  testWidgets('groups what would go by section, with its total', (tester) async {
    await tester.pumpWidget(harness(_FakeCleanRepository([
      const CleanPlan(candidates: [
        CleanCandidate(
    ...
    expect(find.text('USER ESSENTIALS'), findsOneWidget); // card titles uppercase
  ...
  testWidgets('says what it left alone, and why', (tester) async {
    await tester.pumpWidget(harness(_FakeCleanRepository([
      const CleanPlan(candidates: [
        CleanCandidate(path: '/a/cache', section: 'User essentials', sizeBytes: 10),
        CleanCandidate(
          path: '/a/keychain',
          section: 'User essentials',
  ...
  testWidgets('nothing moves until the confirmation is accepted', (tester) async {
  ...
  testWidgets('confirming moves exactly what the plan proposed', (tester) async {
    final repository = _FakeCleanRepository([
      const CleanPlan(candidates: [
        CleanCandidate(
          path: '/Users/tester/Library/Caches/app',
  ...
  testWidgets('the button is disabled when there is nothing to do', (tester) async {
    await tester.pumpWidget(harness(_FakeCleanRepository([
      const CleanPlan(candidates: []),
    ])));
    await tester.pumpAndSettle();
