/// Bundle-identifier patterns that cleanup must never touch, ported verbatim
/// from Mole's `lib/core/app_protection_data.sh` so the two stay comparable
/// entry for entry. Data only — the rules that consume these live in
/// `path_protection.dart`.
///
/// Matching is case-sensitive shell-glob, the same as Mole's
/// `bundle_matches_pattern`: macOS system bundles ship inconsistent casing
/// across releases, so an entry is only as good as its exact spelling.
library;

/// Components the system itself needs. Removing their data breaks
/// macOS, not just an app.
const systemCriticalBundles = <String>[
  'com.apple.finder',
  'com.apple.dock',
  'com.apple.Safari',
  'com.apple.mail',
...
/// Apps whose "cache-shaped" directories hold licences, sessions,
/// accounts or input-method state — regenerating them costs the user
/// real data, not just time.
const dataProtectedBundles = <String>[
...
/// EDR/MDM agents that tamper-protect their on-disk state. Touching
/// anything of theirs under the per-user Darwin folder trips sensor
/// tamper detection that corporate security reports as malware.
const endpointSecurityBundlePrefixes = <String>[
  'com.crowdstrike.',
  'com.sentinelone.',
  'com.sentinel-labs.',
  'com.eset.',
